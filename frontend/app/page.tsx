...
// MetaMask exists but may be locked
try {
  ...
  alert("MetaMask detected but not accessible. Please unlock MetaMask.");
  return;
}
...
async function requestAirdrop() {
  ...
  setStatusMessage("⏳ Sending 100 PINN airdrop...");
  ...
  try {
    ...
    setAirdropDone(true);
    setStatusMessage("🎉 100 PINN sent successfully!");
    localStorage.setItem("pinn_airdropped", "true");

    try {
      await addPINNToMetaMask();
    } catch {
      console.warn("Token add skipped");
    }

  } catch (err: any) {
    setStatusMessage(err.message || "❌ Airdrop failed. Please try again.");
  }
}

// MVP airdrop (admin-funded, first signup only)
if (!localStorage.getItem("pinn_airdropped")) {
  try {
    setStatusMessage("⏳ Sending 100 PINN tokens...");
    ...
    localStorage.setItem("pinn_airdropped", "true");
    setStatusMessage("🎉 100 PINN tokens sent successfully!");
  } catch (err: any) {
    console.error(err);
    setStatusMessage(
      err.message ||
        "Airdrop failed. Please ensure Polygon Mainnet is selected."
    );
  }
}
  }
  ...
  try {
    await window.ethereum.request({
      ...
      params: {
        type: "ERC20",
        options: {
          address: process.env.NEXT_PUBLIC_PINN_CONTRACT_ADDRESS!,
          symbol: "PINN",
          decimals: 18,
          image: "https://the-big-picture.info/pinn44-logo.jpg"
        }
      }
    });
  } catch (error) {
    console.error("Failed to add PINN to MetaMask", error);
  }
}
